#подключение модуля socket для получения ip-адркса машины, на которой интерпретатор Python в настоящее время выполняется.
import socket
#аолкючение класса Flask из модуля flask
...
# вызов метода run объекта app: запуск приложения в режиме дебага
app.run(debug = True)
